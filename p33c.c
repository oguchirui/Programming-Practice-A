...
int main(void){
    ...
    printf("%.3f %.3f %.3f\n",F(b)-F(a),integral_trapezoid(fp,a,b,n),integral_Simpson(fp,a,b,n)); //不定積分関数から求めた定積分、台形則の関数から求めた定積分、Simpson則の関数から求めた定積分の結果を出力
...
double integral_Simpson(double (*f)(double x),double a,double b,int n){ //Simpson則の関数を定義
    double h=(b-a)/n; //範囲[a,b]をn等分する
    double S=0; //足し合わせた面積

    S+=h*((double)1/(double)3)*f(a); //第1項の計算
    n-=1; //計算後、nを1ずつ減らしていき、奇数番目項、偶数番目項を判断

    for(double x=a+h;x<b-h+0.01*h;x+=h){ //第2~n-1項の計算
        if(n%2==1){
            S+=h*((double)4/(double)3)*f(x); //奇数番目項の計算
            n-=1;
        }

        else{
            S+=h*((double)2/(double)3)*f(x); //偶数番目項の計算
            n-=1;
        }
    }

    S+=h*((double)1/(double)3)*f(b); //第n項の計算
